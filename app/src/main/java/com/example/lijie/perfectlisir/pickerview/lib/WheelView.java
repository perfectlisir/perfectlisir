package com.example.lijie.perfectlisir.pickerview.lib;
...
import com.example.lijie.perfectlisir.pickerview.adapter.WheelAdapter;
...
import com.example.lijie.perfectlisir.pickerview.utils.ScreenUtils;
...
public class WheelView extends View
{
	...
	// 条目间距倍数
	static final float lineSpacingMultiplier = 1.7F;//1.4F
	boolean isLoop;
	...
	// 文字最多显示个数
	private int mostShowTextNum = 4;
	// 屏幕宽度
	private int screenWith = ScreenUtils.getScreenWidth(getContext());
	...
	protected void onDraw(Canvas canvas)
	{
		...
		{
			L.e("出错了！adapter.getItemsCount() == 0，联动数据不匹配");
		}
		if (!isLoop)
		{// 不循环的情况
...
//			canvas.drawText(label, drawRightContentStart - CENTERCONTENTOFFSET, centerY, paintCenterText);
			canvas.drawText(label, (drawRightContentStart - (int)(measuredWidth*0.15f)), centerY, paintCenterText);
		}
		counter = 0;
		while (counter < itemsVisible)
		{
			...
			{
				String contentText;
				if(measuredWidth < screenWith * 0.4){
					contentText = getSpecialNumText(getContentText(visibles[counter]), mostShowTextNum);
				}else {
	...
	private String getSpecialNumText(String contentText, int specialNum){
		if(contentText.length() > specialNum){
			StringBuilder stringBuilder =  new StringBuilder(contentText);
			stringBuilder.delete(specialNum,contentText.length()).append("…");
